...
| 2b. Strategy and model design | `meta-business-model-design`; `meta-strategic-factor-analysis`; `meta-strategic-options-evaluation`; `meta-strategic-audit` for an existing organisation; `meta-international-market-entry` when crossing borders | GEL scored model and final evaluation; EFAS/IFAS/SFAS on one stated 1-5 scale; TOWS; SAFe screen; 35-word strategy statement | Model passes the GEL final evaluation (or a documented owner exception); strategy statement passes the gate; rejected options recorded | Block sections 01-16 until the model and statement pass; restart at the failed lever or option |
